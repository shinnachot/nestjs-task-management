...
- **`POST /auth/refresh`** → body `{ refreshToken }`, returns `{ accessToken, refreshToken }` (rotates refresh token)
- **`POST /auth/logout`** → requires access token, clears stored refresh token hash
